...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, stride=32, pano_dims=None):
    ...
    def __next__(self):
        ...
        # Padded resize
        if self.pano_dims is not None:
            split_count = 4
            imgs = [img0[ self.pano_dims[1]// 2 : self.pano_dims[1] // 2 * 3,i*self.pano_dims[0]:(i+1) * self.pano_dims[0]] for i in range(split_count)]
            imgs = [letterbox(img, self.img_size, stride=self.stride)[0] for img in imgs]
        else:
            imgs = [letterbox(img0, self.img_size, stride=self.stride)[0]]

        # Convert
        imgs = [img[:, :, ::-1].transpose(2, 0, 1) for img in imgs]  # BGR to RGB, to 3x416x416
        imgs = [np.ascontiguousarray(img) for img in imgs]

        return path, imgs, img0, self.cap
