...
def detect(save_img=False):
    source, weights, view_img, save_txt, imgsz, trace, blurratio,hidedetarea, pano = opt.source, opt.weights, opt.view_img, opt.save_txt, opt.img_size, not opt.no_trace, opt.blurratio,opt.hidedetarea, opt.pano
    if pano:
        pano_dims = (3072, 3072)
    else:
        pano_dims = None
    ...
    if webcam:
        ...
        dataset = LoadImages(source, img_size=imgsz, stride=stride, pano_dims=pano_dims)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pano',action='store_true', help='detect in panorama, only uses vertical middle and splits in 4 images of half height.')
